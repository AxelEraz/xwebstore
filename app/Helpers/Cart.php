...
class Cart
{
    ...
    public function down(Product $product): void
    {
        ...
        if (in_array($product->id, $cartProductsIds)) {
            $cart['products'] = $this->productCartDecrement($product->id, $product->price , $cart['products']);
    ...
    private function productCartDecrement($productId, $productPrice, $cartItems)
    {
        $amount = -1;
        $price = - $productPrice;
        $cartItems = array_map(function ($item) use ($productId, $amount, $price) {
            if 
            ($productId == $item['id']) {
                $item['amount'] += $amount;
                $item['price'] += $price;}     
